 ...
 *
 * Specs (mirroring Menu Item / Command Item pattern):
 *   Content: rounded-md, border=--border, bg=--card, p=4, shadow-md
 *   Item: rounded-md (6px), pad=[8,6,8,6] (px-xs py-2xs), gap=8 (gap-xs), text 14px/400
 *   Focus: bg=--accent
 *   Separator: 1px --border
 *   Shortcut: text-muted-foreground, 12px
 */
...
function ContextMenuSubTrigger({
  ...
  return (
    <ContextMenuPrimitive.SubTrigger
      ...
      className={cn(
        "flex cursor-default select-none items-center gap-xs rounded-md px-xs py-2xs typo-paragraph-sm outline-none focus:bg-accent focus:text-foreground data-[state=open]:bg-accent data-[state=open]:text-foreground [&_svg]:pointer-events-none [&_svg]:size-md [&_svg]:shrink-0",
        inset && "pl-2xl",
        className
      )}
      {...props}
